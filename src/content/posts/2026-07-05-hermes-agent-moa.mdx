...
description: "封面：Hermes Agent × 多模型 pipeline——dark premium editorial，brand mark + 模型 logo + 天平 verdict。"
...
![Collaborativeness：双区 info-graphic——"COLLABORATIVENESS" + 副标 "Even weaker models help a stronger aggregator."，左区 VISUAL：3 个大小不一的 reference 圆（gpt-5.5 蓝大 / deepseek-v4-pro 赭中 / grok-4 青小=最弱）汇入中央深炭灰 AGGREGATOR（claude-opus-4.8），最弱节点虚线注释 "weaker, but still contributes"；右区 DATA 面板 "EVIDENCE · ALPACAEVAL 2.0"：Open-source MoA 65.1%（赭色高亮）vs GPT-4o 57.5%，强调 +7.6 pts LC win rate gap。flat 编辑信息图。](./assets/2026-07-05-hermes-agent-moa/collaborativeness.png)
...
![Concept：transparency 信息图——标题 "TRANSPARENCY" 与副标 "Watch the committee deliberate, not just read the verdict."，下方 3 个标注的 reference 面板（REFERENCE · gpt-5.5 / deepseek-v4-pro / grok-4，抽象横线代表推理文本），"AGGREGATING" 分隔，底部 VERDICT · claude-opus-4.8 面板（更密的合成文本）。暖米色背景，干净编辑信息图风。](./assets/2026-07-05-hermes-agent-moa/concept-deliberation.png)
...
![Failure isolation：信息图——标题 "FAILURE ISOLATION" 与副标 "One reference fails. The committee still delivers a verdict."，3 个 reference 中 gpt-5.5 淡化显示 + 红色 FAILED 徽章、无输出流，deepseek-v4-pro 与 grok-4 仍发射流汇入 AGGREGATOR（claude-opus-4.8）产出 VERDICT（带法槌图标）。暖米色背景，干净编辑信息图。](./assets/2026-07-05-hermes-agent-moa/failure-isolation.png)
